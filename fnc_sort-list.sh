...
for lset in $lsets ; do
	for src_child in $($s_zfs list -Hr -o name $lset) ;do
		...
		if [ "$excl" = "1" ] ;then
					...
					[ "$cln" ] || include_Array+=([$src_child]=d)
					[ "$cln" ] && include_Array+=([$src_child]=cl)
					[ "$cln" ] && temp_clone_array+=($src_child)
					[ "$cln" ] && temp_clone_Array+=([$src_child]="$clone")
					dest_Array+=([$src_child]="$dest_child")
